...
bool starcoder_model_load(const std::string & fname, starcoder_model & model, gpt_vocab & vocab, bool verbose = false) {
    if (verbose) printf("%s: loading model from '%s'\n", __func__, fname.c_str());
...

    {
        ...
        if (verbose) printf("%s: ggml ctx size = %6.2f MB\n", __func__, ctx_size/(1024.0*1024.0));
    }
    ...
    // key + value memory
    {
        ...
        if (verbose) printf("%s: memory size = %8.2f MB, n_mem = %d\n", __func__, memory_size/1024.0/1024.0, n_mem);
    }
    ...
    // load weights
    {
        ...
        if (verbose) printf("%s: model size  = %8.2f MB\n", __func__, total_size/1024.0/1024.0);
    }
...
void starcoder_call(gpt_params& params,
    ...
    // load the model
    {
        ...
        if (!starcoder_model_load(params.model, model, vocab, verbose)) {
    ...
    for (int i = 0; i < embd_inp.size(); i++) {
        if (verbose) printf("%s: token[%d] = %6d, %s\n", __func__, i, embd_inp[i], vocab.id_to_token.at(embd_inp[i]).c_str());
    }
    printf("\n\n");
    ...
    for (int i = embd.size(); i < embd_inp.size() + params.n_predict; i++) {
        ...
        // display text
        for (auto id : embd) {
            std::string s = std::string(vocab.id_to_token[id]);
            output += s;
            if (print_response) printf("%s", vocab.id_to_token[id].c_str());
        }
        fflush(stdout);
